...
- The one-shot reconciliation command refreshes Airbyte run history while no user is signed in, then normalizes only sources whose newest observed synchronization succeeded. It runs beside the persistent services under the host scheduler with overlap disabled; Vercel serves the control plane and does not own this background workload.
- After warehouse account scopes are published, Mosaic now requests the first Airbyte sync immediately through the documented Jobs API instead of waiting for the next cron boundary. The accepted job is recorded as pending or running before redirecting, duplicate active-job responses are treated as replay-safe, and later reconciliation replaces the provisional timestamp with Airbyte's authoritative run data.
- Superadmins can request an on-demand GA4 sync from the authorization catalog without supplying an Airbyte connection identifier. The action validates the Mosaic authorization UUID, re-resolves the active server-side connection, records a safe audit outcome, and treats Airbyte's already-running response as success.
- GA4 source registration now transactionally retains the selected property boundary before connection creation. If stream discovery or connection creation fails after OAuth, the authorization catalog exposes a superadmin-only retry that reuses Airbyte's existing credential-bearing source, recreates only the missing connection, republishes canonical scopes, and requests the initial sync without another Google authorization.
- If that incomplete source cannot be recovered, the same catalog can delete the credential-bearing Airbyte source without pretending a warehouse connection existed. Mosaic retains a revoked local tombstone so warehouse scopes can be deactivated and audit history remains attributable. Partial remote cleanup remains visible for operator intervention.
- If local registration fails immediately after Airbyte source creation, the callback compensates by deleting that source through the source-only cleanup path. A failed compensation is surfaced explicitly for private Airbyte inspection instead of leaving a silent credential orphan.
