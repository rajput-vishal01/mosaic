      ...
      <div className="mx-auto max-w-7xl px-5 py-8 lg:px-8">
        {agency?.status === "suspended" ? (
          ...
          <>
            ...
            {!superadmin && !isAgencyAdmin(agency?.role) && <section className="mt-6 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"><div className="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="font-semibold text-slate-900">Assigned accounts</h2><p className="mt-1 text-sm text-slate-500">Only these source accounts will be included in your dashboards and reports.</p></div>{grantedAccounts.some((account) => account.provider === "ga4") && <Link className="rounded-lg bg-slate-950 px-3 py-2 text-sm font-semibold text-white hover:bg-slate-800" href="/dashboard/analytics">Open analytics</Link>}</div>{grantedAccounts.length === 0 ? <p className="px-5 py-10 text-center text-sm text-slate-500">No accounts have been assigned yet.</p> : <div className="divide-y divide-slate-100">{grantedAccounts.map((account) => <div className="flex flex-col gap-2 px-5 py-4 sm:flex-row sm:items-center sm:justify-between" key={account.id}><div><p className="font-medium text-slate-900">{account.name}</p><p className="mt-1 text-sm text-slate-500">{providerLabels[account.provider as ProviderKey]} · {account.agencyName}</p></div><div className="sm:text-right"><span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${accountHealthTone[account.health.state]}`}>{account.health.label}</span>{account.lastSuccessfulAt && <p className="mt-1 text-xs text-slate-500">Data through {account.lastSuccessfulAt.toLocaleString()}</p>}</div></div>)}</div>}</section>}
