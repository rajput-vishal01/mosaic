    ...
    <main className="min-h-screen bg-slate-50 px-5 py-8 lg:px-8">
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mt-6 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          ...
          {connections.length === 0 ? <p className="px-5 py-10 text-center text-sm text-slate-500">No authorization records exist yet.</p> : <div className="divide-y divide-slate-100">{connections.map((connection) => <div className="flex flex-col gap-3 px-5 py-4 sm:flex-row sm:items-center sm:justify-between" key={connection.id}><div><div className="flex flex-wrap items-center gap-2"><p className="font-medium text-slate-900">{providerLabels[connection.provider as ProviderKey]}</p><span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600">{connection.externalReference ? "Service linked" : "Fixture"}</span></div><p className="mt-1 text-sm text-slate-500">{connection.label}</p></div><div className="text-sm text-slate-500 sm:text-right"><p>{connection.accountCount} {connection.accountCount === 1 ? "account" : "accounts"}</p><p className="mt-1 text-xs capitalize">{connection.status}</p></div></div>)}</div>}
        </section>
      </div>
    </main>
  );
