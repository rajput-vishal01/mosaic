    ...
    <main className="min-h-screen bg-slate-50 px-5 py-8 lg:px-8">
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mt-6 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          ...
          {connections.length === 0 ? <p className="px-5 py-10 text-center text-sm text-slate-500">No authorization records exist yet.</p> : <div className="divide-y divide-slate-100">{connections.map((connection) => <div className="flex flex-col gap-3 px-5 py-4 sm:flex-row sm:items-start sm:justify-between" key={connection.id}><div><div className="flex flex-wrap items-center gap-2"><p className="font-medium text-slate-900">{providerLabels[connection.provider as ProviderKey]}</p><span className={`rounded-full px-2 py-0.5 text-xs font-medium ${connectionHealthTone[connection.health.state]}`}>{connection.health.label}</span></div><p className="mt-1 text-sm text-slate-500">{connection.label} · {connection.externalReference ? "Service linked" : "Fixture catalog"}</p><p className="mt-2 max-w-xl text-xs leading-5 text-slate-500">{connection.health.message}</p></div><div className="flex flex-col items-start gap-3 text-sm text-slate-500 sm:items-end sm:text-right"><div><p>{connection.accountCount} {connection.accountCount === 1 ? "account" : "accounts"}</p>{connection.lastSuccessfulAt && <p className="mt-1 text-xs">Last sync {connection.lastSuccessfulAt.toLocaleString()}</p>}</div>{connection.provider === "ga4" && connection.status === "active" && connection.airbyteSourceId && connection.airbyteConnectionId && <div className="flex flex-wrap items-start gap-2 sm:justify-end"><ConnectionSyncForm authorizationId={connection.id} /><RevokeConnectionDialog authorizationId={connection.id} label={connection.label} /></div>}</div></div>)}</div>}
        </section>
